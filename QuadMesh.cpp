...
bool
QuadMesh::InitMesh(int meshSize, VECTOR3D origin, double meshLength, double meshWidth, VECTOR3D dir1, VECTOR3D dir2,
                   Blob blob) {
    ...
    for (int i = 0; i < meshSize + 1; i++) {
        for (int j = 0; j < meshSize + 1; j++) {
            ...
            // If reset is called, then the plane is initialized with y-values at 0.
            if (reset) {
                vertices[currentVertex].position.Set(meshpt.x, 0, meshpt.z);
            }
                // Other wise add the metaballs influence to each y-component of the vertices.
            else {
...
/// Generates the height of the blob per given vertex. Implements
/// Guassian metaballs formula with added noise.
float QuadMesh::getInfluenceHeight(Blob &blob, const VECTOR3D &meshpt) {
    float height = 0.0;
    if (!blob.isEmpty()) {
        float r = sqrt(pow((blob.getX() - meshpt.x), 2) + pow((blob.getZ() - meshpt.z), 2));
        if (r != 0) {
            float ran = RandomFloat(0.5, 1.0);
            height += (blob.getHeight() * exp(-blob.getWidth() * pow(r, 2))) * ran;
        }
    }
    return height;
}

/// Used to add noise to terrain. Generates random
/// floats within a certain range.
/// @param float min Minimum value of interval.
/// @param float max Maximum value of interval.
/// @return Random float within given interval.
